...
import { adjustStockAction } from "@/app/(admin)/admin/(authenticated)/inventory/actions";
...
function InventoryRow({ row, needsReorder }: { row: InventoryOverviewRow; needsReorder: boolean }) {
  ...
  return (
    <tr>
      ...
      <td>
        ...
        {row.recommendedReorderQty ? ` (reorder ${row.recommendedReorderQty})` : ""}
      </td>
      <td>
        {needsReorder && <span className="sd-badge sd-badge-warning">Reorder</span>}
      </td>
      <td>
        <form action={formAction} style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
          ...
          />
          <input type="text" name="reason" placeholder="Reason (optional)" className="sd-inline-field" style={{ width: "9rem" }} />
          <SaveButton />
        </form>
      ...
      <table className="sd-table">
        <thead>
          <tr>
            ...
            <th></th>
            <th>On hand / adjust</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <InventoryRow key={row.inventoryItemId} row={row} needsReorder={reorderProductIds.has(row.productId)} />
          ))}
        </tbody>
      </table>
    </div>
